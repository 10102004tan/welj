...
import audio49 from "../assets/49.mp3";
...
export const PlayCircleIcon = ({ size = 24, width, height, ...props }) => {
  return (
    <svg
      ...
      width={size || width}
      {...props}
      xmlns="http://www.w3.org/2000/svg" class="lucide lucide-play-icon lucide-play"
...
export default function MusicPlayerBar({
  audioSrc = MinutesEasy,
  audioTitle = "ESL 52010",
  audioDescription = "E10: Choosing a film",
  audioCover = "https://heroui.com/images/album-cover.png",
}) {
  ...
  const rewindTime = 5; // seconds to rewind
  const SPEED = [
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.code === SHORTCUTS.play || e.code === SHORTCUTS.pause) {
        handlePlay();
      } else if (e.code === SHORTCUTS.next) {
        duration - currentTime > rewindTime ? audioRef.current.currentTime += rewindTime : audioRef.current.currentTime = duration;
        audioRef.current.currentTime += rewindTime
      } else if (e.code === SHORTCUTS.previous) {
        audioRef.current.currentTime -= rewindTime
      }
    };
  ...
  return (
    <div className="w-full grid px-3 grid-cols-5 fixed bottom-0 left-0 z-50  bg-white/30 backdrop-blur-md shadow-2xl py-3">

      <div className="flex items-center gap-2 px-4 col-span-1">
        <img
          alt="Album cover"
      ...
      <div className="col-span-3">
        <div className="flex w-full items-center justify-center">
          ...
          <Button
            isIconOnly
            className="w-[50px] text-center h-[40px] data-[hover]:bg-foreground/10"
            radius="full"
            variant="light"
          >
            {
              isPause ? (
                <PauseCircleIcon size={40} onClick={handlePlay} />
              ) : (
          ...
          {/* speed */}
          <select onChange={handleChangeSpeed} className="ml-2 text-sm text-gray-500 bg-transparent border-none outline-none cursor-pointer">
            {SPEED.map((item) => (
              // set default = 1
              <option key={item.key} value={item.key} selected={item.key === "1"}>{item.label}</option>
            ))}
          </select>
        </div>
        <div className="flex mt-3 gap-2 justify-between items-center">
          <audio preload="metadata" className="hidden" ref={audioRef} src={audioSrc} />
          <p className="text-small">
            {formatTime(currentTime)}
          </p>
          <input type="range" min="0" max="100" value={progress} onChange={handleSeek} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700" />
